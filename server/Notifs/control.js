...
export const ListNotifs = async (req, res) => {
	const API = new ApiView(NOTIFS, { user_id: req.user._id }, req.query),
		{ status, msg, data } = await API.exec(
			API.list(req.query.search, ['title', 'message'], req.query.start, req.query.end, req.query.page)
		);
...
export const ReadNotif = async (req, res) => {
	const API = new ApiView(NOTIFS, { _id: req.params.key, user_id: req.user._id, is_read: false }),
		{ status, msg, data } = await API.exec(API.update({ is_read: true }));
...
export const CountNotif = async (req, res) => {
	const API = new ApiView(NOTIFS, { user_id: req.user._id, is_read: false }),
		{ status, msg, data } = await API.exec(API.count());
